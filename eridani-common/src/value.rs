use crate::{bytecode::Method, internal_error};
...
impl Hash for Value {
    fn hash<H: hash::Hasher>(&self, state: &mut H) {
        ...
        match self {
            ...
            Value::Nothing => {}
            Value::Method(method) => (&raw const **method).hash(state),
...
impl FromStr for Value {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if let Ok(n) = s.parse() {
            ...
            && s.len() > 1
        {
            Ok(Value::String(s[1..s.len() - 1].into()))
        } else if s == "nothing" {
            Ok(Value::Nothing)
        } else if s.starts_with('[') && s.ends_with(']') {
            let s = &s[1..s.len() - 1];
...
impl Type {
    #[must_use]
    pub fn is_kind(self, value: &Value) -> bool {
        match (self, value) {
