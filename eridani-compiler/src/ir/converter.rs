use crate::{
    ir::{
        ...
        BinOp, Binding, Environment, Expr, Function, ListExpr, Method, Pattern, PlaceHolderModule,
        Program, UnOp,
    },
...
use eridani_common::{internal_error, natives};
...
fn get_std() -> Result<(Vec<Rc<RefCell<Module>>>, HashMap<String, Binding>)> {
    let mut eridani_std = crate::eridani_std::ERIDANI_STD_BASIC.to_owned();

    #[cfg(feature = "target-std")]
    (eridani_std += crate::eridani_std::ERIDANI_STD_FEATURE_STD);
    ...
    for function in natives::NATIVES {
        natives_module.push_function(Rc::new(RefCell::new(Function::Rust(function))));
    }
    ...
    let prelude = crate::eridani_std::PRELUDE;
    let mut bindings = HashMap::new();
    bindings.insert("std".to_owned(), Binding::Module(Rc::clone(&std_module)));
    for function in std_module.borrow().functions() {
        if prelude.contains(&function.borrow().name()) {
            bindings.insert(
                function.borrow().name().to_owned(),
...
pub(super) fn convert(
    ...
    for module in &modules {
        ...
        for function in module.borrow().functions() {
            if !all_functions.iter().any(|f| Rc::ptr_eq(f, function)) {
...
fn analyse_method(
    ...
    let Ok(arg_order) = resolve_metapatterns(&args) else {
...
fn convert_variable(
    ...
    {
        Expr::PlaceHolder(var.clone(), PlaceHolderModule(Rc::clone(module)))
...
fn convert_expr(
    ...
    let result = match expr {
        ParseExpr::Binary {
            ...
            let left = convert_expr(left, module, Rc::clone(&environment), modules)?;
        ...
        ParseExpr::Call {
            ...
            let callee = convert_expr(callee, module, Rc::clone(&environment), modules)?;
...
fn convert_exprs(
    ...
    let mut result = vec![];
    for value in values {
        let expr = convert_expr(value, module, Rc::clone(environment), modules)?;
