...
use eridani_common::{bytecode::Program, discriminant, value::Value, ArgumentError};
...
impl Error {
    fn from_argument_error(error: ArgumentError, function_name: String) -> Self {
        Error {
    ...
    const fn new(kind: String, message: String, line: usize) -> Self {
        Error {
...
pub fn run(program: Program, args: Vec<Value>) -> Result<Value> {
    if !discriminant::compatible_with_runtime(&program) {
        return Err(Error::new(
            "Compatability".to_owned(),
            "Not all expected features are enabled in the runtime".to_owned(),
            0,
        ));
    }
