...
impl Vm {
        ...
        match op_code {
            ...
            GenericOpCode::Negate => {
                ...
                let result = (-&value).ok_or_else(|| {
                    self.error("Arithmetic".to_owned(), format!("Cannot negate {value}"))
                })?;
                self.stack.push(result);
            }
        ...
        match callee {
            Value::Function(function) => {
                if function.kind == FunctionKind::Eridani {
                    ...
                    let native = &mut self.natives[usize::from(function.reference)];
                    let value = (native.fun)(&self.stack[args_start..]).map_err(|error| {
                        Error::from_argument_error(error, native.name.to_owned())
                    })?;
            ...
            _ => return Err(self.error("Type".to_owned(), format!("Cannot call '{callee}'"))),
        }
        ...
        match op_code {
            ...
            ExprOpCode::Spread => {
                ...
                if let Value::List(mut list) = value {
                    ...
                    return Err(self.error(
                        "Type".to_owned(),
                        "Cannot use '..' on a non-list".to_owned(),
                    ));
                }
            }
            ExprOpCode::Pattern => {
    ...
    fn concat_compare_value_waiting(&mut self) -> Option<bool> {
        ...
        if let Some(value_start) = base.find(&value) {
            let var = Value::String(base[match_start..value_start].to_owned());
        ...
        match op_code {
            ...
            PatternOpCode::EndConcatWithVar => {
                ...
                self.current_callframe_mut()
                    .variables
                    .push(Value::String(value[match_start..].to_owned()));
                self.stack.push(Value::Number(1.0));
                self.current_callframe_mut().concatenation_match_start = None;
            }
